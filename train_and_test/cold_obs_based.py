...
def get_movie_ids(training_df, testing_df):
    obs_per_movie = testing_df[~testing_df.movieId.isin(training_df.movieId)].movieId.value_counts()
    ...
    if obs_per_movie.shape[0] > args.n_items:
        ...
        logging.info("The number of candidates [%s] < n_items. Taking them all", obs_per_movie.shape[0])
...
def get_training_and_testing_dfs(rating_csv, split_q):
    ...
    testing_df = clean_bad_uids_from_df(testing_df[testing_df.userId.isin(training_df.userId)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', default=5, dest="n_obs", type=int,
                        help='The number of observations for cold-start item '
                             'in the test set. Default: 5')
